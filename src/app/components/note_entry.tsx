...
export default function Entry(
    ...
    const putData = async (likes: number) => {

        const contentType = 'application/json'
        const post = { likes: likes }
    ...
    return (
        <div onClick={handleClick} className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-2 sm:p-3 m-2 sm:m-4 h-fit float-left">
            <h1 className=" dark:text-white text-[1rem] sm:text-[1.2rem] mb-2 break-words">{title && title.length > 30 ? title.substring(0, 30) + "..." : title}</h1>
            ...
            <div className=" flex justify-between items-center my-1">
                <div className="flex justify-around items-center w-1/3 sm:w-[30%] ">
                    <p className="text-[0.5rem]">{isLike}</p>
                    {isLiked ?
                    ...
                    <p className="text-[0.5rem]">{comments.length}</p>
