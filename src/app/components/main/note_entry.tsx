...
export default function Entry(
  ...
  useEffect(() => {
    ...
    if (status === "authenticated") {
      const showLike = likes && likes.includes(currentUser)
  ...
  return (
    <div onClick={handleClick} className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-2 sm:p-3 m-2 sm:m-4 h-fit float-left">
      ...
      <p className="text-right text-xs text-slate-500">~ {name.length > 0 && name}</p>

      <div className=" flex justify-between items-center my-1">
        <div className="flex justify-around items-center w-2/5 sm:w-1/3 ">
          <p className="text-[0.5rem]">{likes && (likes.length !== 0) && likes.length}</p>
          {isLiked ?
          ...
          <p className="text-[0.5rem]">{comments && (comments.length !== 0) && comments.length}</p>
